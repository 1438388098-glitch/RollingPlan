...
### v0.13 → v0.28（当前 v0.28）

| 版本 | 内容 |
|------|------|
| v0.29 | **全局动效**（`animations.py` 统一收口）：切页淡入 / 折叠区高度展开·收起（执行页「更多」、队列行、制定页五组、归档页「⋯」）/ 完成并滚动·切天·切分类 的轻淡入反馈 / 额外安排按钮·归档「今天完成」·制定页预览区 浮现 / 额外安排对话框淡入。统一 OutCubic + 150~220ms，动完即摘特效；**`QT_QPA_PLATFORM=offscreen` 下自动禁用**（测试与 dump 输出零变化） |
...
- 每一格：时段名 + 计划内容 + **一个**「✓ 完成并滚动」
  - **右键这一行** = 仅完成 / 固定计划 / 拦截滚动 / 完成并滚动
...
11 个测试文件，`bash run_all_tests.sh` 一把跑（退出码 0 = 全过 / 1 = 有失败 / 2 = 环境不对）。
