...
**v0.12** — 20 commits on `main`，已全部推送
...
**6 个测试文件 / 总计 341 个断言全过，0 失败**（2026-09-15 23:35 于验收副本 .venv 实测）
...
**选项 C**：先问用户 v0.12 在真机上用起来对不对（**headless 测试 = 341 断言全过，
但视觉/手感没有真机确认** —— 参见下方备忘）。这一版每格 4 个按钮，**行宽可能不够**；
新增的「📋 额外安排」细长条按钮和列表对话框也只是几何校验过，没真机看过
