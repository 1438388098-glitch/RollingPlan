...
> **接手先读本文件**：项目状态都记在这儿（版本 / 改动 / 测试 / 路径 / 待办 / 坑），不用去翻别的记录
...
**还没定的一件事**（见「待确认」）：v0.11/v0.12 都是 headless 测过、**没在真机上跑过**；
另外「上滚时后面的计划可以越过被固定的那一格」这个语义是按理解先定的，等确认。
...
**前置**：无需 push（已与 `origin/main` 同步）。直接读本文件即可接着做 —— 建议先 `cd`
进仓库，跑一遍下面那条命令确认 341 断言全过，再动代码。

**第一优先（等确认）**：v0.12 真机验收。三个检查点：
...
- 「固定计划」目前允许后面的计划**越过**它去填前面的空位（早1中2晚3，固定中，滚早 → 早3 中2 晚空）。
  另一种读法是「被固定那格变成墙，后面的也不能越过」→ 那就是 早空 中2 晚3。等确认
- 「直接拉取」现解成「列表里一个按钮，不挑、按顺序拉下一条」。若是
